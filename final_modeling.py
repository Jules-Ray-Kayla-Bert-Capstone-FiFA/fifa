...
def create_baseline(y_train, y_validate, target):
    ...
    print("RMSE using Mean\nTrain/In-Sample: ", f'€{rmse_train:.2f}', 
          "\nValidate/Out-of-Sample: ", f'€{rmse_validate:.2f}')
...
# def create_test_baseline(y_test, target):
    
#     # We need y_train and y_validate to be dataframes to append the new columns with predicted values. 
#     y_test = pd.DataFrame(y_test)
    
#     # 1. Predict wage_eur_pred_mean
#     wage_eur_pred_mean = y_test['wage_eur'].mean()
#     y_test['wage_eur_pred_mean'] = wage_eur_pred_mean
...
def tweedie_model(y_train,y_validate,X_train,X_validate):
    ...
    print("RMSE for GLM using Tweedie, power = 1 & \u03B1 = 0\nTraining/In-Sample: ", f'€{rmse_train_glm:.2f}', 
...
def actual_vs_predicted(train,y_train,y_validate,X_train,X_validate,y_test):
    ...
    plt.hist(y_validate.wage_eur, color='blue', alpha=.5, label="Actual Final Salary (€)")
    plt.hist(y_validate.wage_eur_pred_lm, color='red', alpha=.5, label="Model: Linear Regression")
    plt.hist(y_validate.wage_eur_pred_glm, color='yellow', alpha=0.5, label="Model: Tweedie Regressor")
    plt.hist(y_validate.wage_eur_pred_lars, color='green', alpha=.5, label="Model: Lasso Lars")
    plt.xlabel("Final wage (€)")
    plt.ylabel("Predicted wage (€)")
    plt.title("Distribution of Actual Weekly Salaries vs. Distributions of Predicted Weekly Salaries for the Top Models")
    plt.legend()
    plt.show()
################################################################    
def test(y_train,y_validate,X_train,X_validate,y_test,X_test):
    ...
    print("RMSE for GLM using Tweedie, power = 1 & \u03B1  = 0\nOut-of-Sample Performance: ", f'€{rmse_test:.2f}')
...
def overall_modeling(df):
    ...
    print("The features used for this model:", mvp)
    X_train = train[mvp]
    ...
    final_modeling.create_baseline(y_train, y_validate,'wage_eur')
    final_modeling.tweedie_model(y_train,y_validate,X_train,X_validate)
    print("Let's take a look at the numbers:")
    print("Train")
    print(y_train)
    print("Validate")
    print(y_validate)
    print("============================================= Test Data ==================================================")
    final_modeling.test(y_train,y_validate,X_train,X_validate,y_test,X_test)
    print(y_test)
    fig = px.histogram(y_test.sample(n=1800, replace=False, random_state=123).sort_index(), labels = {"value": "Salary (€)",
                     "variable": "Actual vs. Predicted",
                     "count": "Count",
                     "wage_eur": "Salary",
                     "wage_eur_pred_glm": "Predicted Salary"
                 },
...
def goalkeeper_modeling(df):
    ...
    final_modeling.test(y_train,y_validate,X_train,X_validate,y_test,X_test)
    print(y_test)
    fig = px.histogram(y_test.sample(n=1000, replace=False, random_state=123).sort_index(),labels = {"value": "Salary (€)",
                     "variable": "Actual vs. Predicted",
                     "count": "Count",
                     "wage_eur": "Salary",
                     "wage_eur_pred_glm": "Predicted Salary"
            ...
            width = 800,
            height = 500)
    fig.show(renderer = 'svg') #renderer="svg"