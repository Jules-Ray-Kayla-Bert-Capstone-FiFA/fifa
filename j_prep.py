...
def prepped_data(df):
    '''This function uses all the functions created to clean and prepare the data for the final outcome of cleaned data.'''
    ...
    # normalize the statistics columns
    df = fix_stats(df)
...
def get_years(df):
    '''This function takes in a dataframe and separates the dataframe by the value in the years column.'''
   ...
   # data for year 2015
    ...
    # data for year 2017
    y_17 = df[(df.year == 2017)]
    # data for year 2018
    y_18 = df[(df.year == 2018)]
    # data for year 2019
    y_19 = df[(df.year == 2019)]

    return y_15, y_16, y_17, y_18, y_19
...
def wrangle_fifa_data(df):
    ...
    df['short_name'] = df['short_name'].replace(['Neymar Jr'],'Neymar')
    df['short_name'] = df['short_name'].replace(['K. Mbappe Lottin'], 'K. Mbappé')
    
#     #normalize stats column
...
def fix_stats(df):
    ...
    #make a list of cols to iterate through
    stats = ['ls', 'st', 'rs', 'lw', 'lf', 'cf', 'rf', 'rw', 'lam', 'cam', 'ram',
           'lm', 'lcm', 'cm', 'rcm', 'rm', 'lwb', 'ldm', 'cdm', 'rdm', 'rwb', 'lb',
           'lcb', 'cb', 'rcb', 'rb', 'gk']
    ...
    for col in copy.columns:
        #create a series to iterate through each col
        series = copy[col]
        ...
        for val in list(series.values):
            try:
                if '+' in val or '-' in val:
                    result[col].append(re.split('\+|\-', val))
                else:
                    result[col].append(val)
            except:
                result[col].append(val)
        
    formatted_cols = pd.DataFrame(result)
    df = df.reset_index()
    df[formatted_cols.columns] = formatted_cols
    ...
    for col in stats:
        ...
        for val in df[col].values:
            ...
            if type(val) == list:
                total[col].append(int(val[0]))
            else:
                total[col].append(int(float(val)))

    df[stats] = pd.DataFrame(total)
...
#     categorical_cols = ['club_position', 'work_rate', 'preferred_foot', 'age_bins', 'weight_bins', 'body_type','league_name']
